...
int main(){
    ...
    /* WE CANMAKE CONSTANT ONLY  PATH NOT  STORAGE*/
    int j = 12;
    int const &k = j;
    // const int &k = j;
    j++;
    // k++;   we cannot increment in k bcoz k is a constant var whch points to j 
    // j can read and write file so it has both access to read and write so we can send copy of j to k var but it is constant so it can only read this cannot write
